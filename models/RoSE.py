...
class PromptContextFusion(nn.Module):
    ...
    def forward(self, prompt, context):
        g = torch.sigmoid(self.Wg1(prompt) + self.Wg2(context))
        fused = g * prompt + (1 - g) * context
        return fused



class RoSE(RobertaPreTrainedModel):
    def __init__(self, config, decode_layer_start=17):
        ...
        elif self.config.architectures[0] == "RoSE":
            self.roberta = RobertaModel_(config, decode_layer_start=decode_layer_start, structural_mask=config.structural_type)
        else:
            pass
        ...
        self.prompt_context_fusion = PromptContextFusion(config.hidden_size)
    ...
    def context_pooling(self, value_matrix, trigger_att, hidden_rep):  # RoIF module
    ...
    def forward(
        ...
        context_outputs_ = self.roberta(
            ...
            return_dict=True,
        structural_mask=torch.stack(enc_attention_mask, dim=0))  # RoGE module
